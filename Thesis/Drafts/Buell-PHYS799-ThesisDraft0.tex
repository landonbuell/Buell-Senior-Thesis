...
\paragraph*{}We can model a chordophone simply as having a string of length $L$, with a constant linear mass density, $\mu$, subject to a tension force $F_T$. The string is flexible, and free to vibrate in a single dimension. We describe the state of string over space and time with a function $U(x,t)$. The behavior of the string can be  modeled by D'Alembert's 1-dimensional wave equation \cite{Haberman,Taylor}:
\begin{equation}
\label{eqn-1DWaveEqn}
...
U(x,t) = \psi(x)h(t)
\end{equation}
Because of the linear nature of the wave equation, Eq. (\ref{eqn-1DWaveEqn}), a linear combination of solutions is also a solution, so we can express $U(x,t)$ as \cite{Haberman}:
\begin{equation}
U(x,t) = 
...
\paragraph*{}Examining the solutions for $U(x,t)$ indicates that under the right initial conditions, chordophones vibrate with all harmonics present.  
