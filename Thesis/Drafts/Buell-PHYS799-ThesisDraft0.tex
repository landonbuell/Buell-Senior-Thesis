...
\paragraph*{}The ability to classify time-series data into unique categories has become topic of much attention in the last few decades \cite{Khan}. These signals can take the form of day-to-day topics such as weather measurements, financial reports, or audio-visual media, and extend into disciplines of medicine, physics, and digital signal processing. In all cases, we explore a problem where the task to categorize the sample, or subset of the sample into one of $k$ \textit{bins} called classes \cite{Mierswa,Virtanen,Geron}.

\paragraph*{}For this classification task, we have developed an automated model (called a \textit{classifier}) which can process information from a time-series audio file, and group the file based on it's prediction of what produced the sound in that file. For example, classifying an audio file which contains the waveform of a clarinet sustaining a single note should return the classification prediction of "clarinet" and so forth. This non-trivial task requires developing a model


 Developments in computational efficiency and the age of \textit{Big Data} has lead to a staggering increase in the utilization
...
\end{equation}
Where $x^{(l)}$ is the final activations for layer $l$ and $z^{(l)}$ is given in equation (\ref{eqn-LinearTransform}). $\sigma^{(l)}$ is some activation function which introduces the ability to account for non-linearity in the 
...
\frac{\partial}{\partial y^*}\Big[ -y \ln (y^*) \Big] = -\frac{1}{y^*_i + \delta}
\end{equation}
...
\end{equation}
Where $z^{(l)}$ are the linear activations defined in Eq. (\ref{eqn-LinearTransform}) and Eq. (\ref{eqn-LinearTransform2}). Note that each $z^{(l)}$ and $x^{(l)}$ are typically stored in memory during the forward pass in Alg. (\ref{algFeedForward}) to prevent the need to recompute them \cite{Geron2,Goodfellow}.
...
\paragraph*{}The spectrogram branch is pictured on the left side of Fig. (\ref{fig-NetworkArchitecture}). A spectrogram is a representation of a sound wave, or signal by representing energy distributions as a function of \textit{time} and \textit{frequency} \cite{White,Olson,Khan}
...
\hline
\textbf{Class Index} & \textbf{Class Name} & \textbf{Count} & \textbf{Class Index} & \textbf{Class Name} & \textbf{Count} \\
\hline \hline
$0$ & Alto Flute & $72$ & & \\ \hline
$1$ & Alto Saxophone & 128&  \\ \hline
$2$ & Banjo &
$3$
$4$
$5$
...
\paragraph*{}From each audio file sample, we derive an identical set of $p$ predictors that will be use for classification. For this multi-modal neural network, the $p$ predictors are arranged into two separated structures, each called a \textit{design matrix}, described in sec. (\ref{subsubsec-DesignMatrix}) \cite{Goodfellow,Geron}.
...
\paragraph*{}This concept stems from the idea that when a trained neural network classifier makes a prediction, it is doing so based on a set of generated \textit{decision boundaries} \cite{Geron,James}.
