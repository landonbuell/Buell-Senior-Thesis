...
\begin{figure}[H]
	...
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/Vibraphone-A5}
	\caption{Vibraphone playing $A5$}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/Marimba-C2}
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/Violin-G4}
	\caption{Violin playing $G4$}
	\end{subfigure}
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/Viola-G4}
...
\paragraph*{}Aerophones all produce sound by exciting molecules of air within a column that vibrate in different patterns called \textit{modes} \cite{White}. The nature of modes are determine by the structure of the instruments. Most aerophones such a flutes, clarinets, and saxophones are more much more reminiscent of air columns that are closed at one end and open at the other. The is marked by the presence of a mouth piece at one end where a played blows air fro their lungs to add energy, and a bell at the other end to serve as an amplification structure \cite{Olson}. For a column of length $L$, that is open at one end, the fundamental frequency is given by \cite{Olson}:
\begin{equation}
f_0 = \frac{v}{4L} = \frac{v}{\lambda}
\end{equation}
where $v$ is the velocity of sound in the medium, and $\lambda$ is the wavelength of the sound wave. This is the lowest pitch that such an instrument can play. Lower woodwinds such as bass clarinets, have a much longer vibrating air column that that of the short piccolo. 
...
\begin{figure}[H]
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/AltoSax-A4}
	\caption{Alto Saxophone playing $A4$}
	\end{subfigure}
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/Clarinet-Gs5}
	\caption{$B\flat$ Clarinet playing $G\sharp 5$}
	\end{subfigure}
	
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/Oboe-Bb4}
	\caption{Oboe playing $B\flat 4$}
	\end{subfigure}
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/Bassoon-D4}
...
\paragraph*{}In addition to the woodwind instruments described above , we also consider the properties of brass aerophones. As the name implies, brass instruments are usually made of interconnected hollow tubes of brass. as opposed to linear columns of mostly wood. brass instruments are also modeled as open-closes columns that vibrate which are again shortened and lengthened by pressing keys. Brass instruments are generally much longer than woodwinds, and produce sounds at a generally greater volume \cite{Olson}. Despite these differences, brass instruments do generally display a great deal of similarities with wood winds, as we can see in Fig. (\ref{fig-PropertiesAerophonesBrass})

\begin{figure}[H]
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/FRENCHHORN-Cs4}
	\caption{French horn playing $c\sharp 4$}
	\end{subfigure}
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/TenorTrombone-F4}
	\caption{Tenor Trombone playing $F4$}
	\end{subfigure}
	
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/Trumpet-B3}
	\caption{$B\flat$ Trumpet playing $B3$}
	\end{subfigure}
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/Tuba-A2}
...
\paragraph*{}Just like with idiophones and chordophones, the amplitude envelop of a brass instrument is very characteristic. Aerophones require a user to vibrate air with their lungs which gradually adds energy to the waveform. This transfer of energy can take comparably long time, giving aerophones particularity long attack times. The note is sustained as long as a player is vibrating the air, allowing for any length of decay and sustain, but the energy does out very suddenly when the player stops, giving a characteristically short release time \cite{White}. 
...
\paragraph*{}We group sounds produced from non-physical musical instruments in this final category. Form our data set, this category includes the four simplest waveforms, sine waves, sawtooth waves, square waves, and triangle waves, as well as any color noises \cite{White}. Since these sounds are only produced synthetically through a MATLAB program, we choose not to include them in the Hornbostel-Sachs grouping above. We explore the properties of each of these generated sounds and how they relate to physical instruments.

\paragraph*{}Each of the five synthetically generated waveforms were produce to be constant with all time. This means that each waveform has no characteristic amplitude envelope and the spectrogram shows no time-evolution of energy in the waveform. Each of the the signals can be composed od a linear combinations of oscillating sinusoidal at various different frequencies and energies. For example, a sine wave conists only of a single oscillating term, while a square wave consists of repeated odd harmonics. These behaviors can be observed in Fig. (\ref{fig-PropertiesSynths}).

\begin{figure}[H]
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/SquareWave-880Hz}
	\caption{Square Wave at $880$ Hz}
	\end{subfigure}
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/SawtoothWave-880Hz}
	\caption{Sawtooth Wave at $880$ Hz}
	\end{subfigure}
	
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/SineWave-880Hz}
	\caption{Sine Wave at $880$ Hz}
	\end{subfigure}
	\begin{subfigure}{0.45\textwidth}
	\centering
	\includegraphics[scale=0.18]{../FiguresSpectrogram/WhiteNoise-190}
	\caption{White Noise}
	\end{subfigure}
\caption{Spectrograms for a few Stringed instruments}	
...
These waveform were included to our data set to serve primilarly as a control to understand how the chosen features respond to simple waveforms. Additionally, the elementary composition of each waveform allows for it to potentially resemble a wide variety of frequency spectra \cite{White}.
