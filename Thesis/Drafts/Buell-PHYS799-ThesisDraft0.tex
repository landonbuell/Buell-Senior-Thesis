...
\paragraph*{}Standard gradient-based learning, as given by the parameter update rule in Eq. (\ref{eqn-GradientLearning}) is cumbersome and often prone to numerical error. This can include phenomena such as \textit{vanishing gradients}, \textit{exploding gradients}, \textit{poor conditioning}, or unstable solution-spaces. \cite{Geron,Goodfellow,Loy}. To combat this, we require a more stable and robust optimizer than the bare gradient descent method. Many gradient-based optimizers have been developed over the years to mitigate some f these vulnerabilities.
...
Note that the upper index $(i)$ or $(i-1)$F gives an iteration index, while the upper iteration $i$ means to raise a value to the power of the iteration. For a given step $(i)$, The ADAM update is given:
\begin{equation}
