...
\paragraph*{}The time domain envelope (TDE) is a rough measurement of the energy containined in the time domain. If we divide the signal into analysis frames, as in Fig. (\ref{fig-AnalysisFrames}), then the TDE approximates the energy in that frame. A frame with a TDE indicates a generally large amplitude, and a frame with a low TDE indicates a generally small ampliute. (i.e. the signal has not started or is decaying)
 
\paragraph*{}We adapt this TDE to \textbf{compute the RMS-Energy of the full waveform}. Since the maximum amplitude of each waveform array has been normalized to $\pm 1$, the TDE presents a consistent measurement of energy in the full time-domain signal \cite{Liu}. The TDE is computed as RMS-Energy of the full waveform, $s$ and is given by \cite{Olson,Serizel}
\begin{equation}
\label{eqn-RMS}
...
\end{center}
\caption{A comparison of the Time Domain Envelope across each class}
