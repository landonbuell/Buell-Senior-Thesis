...
class ProgramMode:
    ...
    def CollectFeatures (self,fileobj):
        ...
        freqFeatures = feat_utils.FrequencySeriesFeatures(timeFeatures.signal,npts=1024)
        featureVector.AddFeatures(freqFeatures.__Call__())              # add frequency-domain features
            
        del(timeFeatures)
        del(freqFeatures)
        return featureVector    # return the feature arrays

    def ConstructDesignMatrices (self,FILES,shapes):
        """ Collect Features from a subset File Objects """      
        shapeX2 = [len(FILES)] + [i for i in shapes[0]] # shape of MLP matrix
        ...
        for i,FILEOBJ in enumerate(FILES):
            ...
            x2 = self.CollectFeatures(FILEOBJ)         # collect features          
        ...
        return X2       # return Design matricies & target matrix
...
class TrainMode (ProgramMode):
    ...
    def TrainModel(self,Networks):
        ...
        for I in range (0,self.n_files,self.groupSize):    # In a given group
            ...
            matrixMLP = self.ConstructDesignMatrices(FILES,shapes)
            
            # Execute training
            modelHistory = Networks.MODEL.fit(  
                    x=matrixMLP.__GetX__(),
                    y=matrixMLP.__GetY__(),
                    batch_size=32,epochs=self.n_epochs,verbose=1
                                                ) 
            # Post-Training
            del(matrixMLP)                      # delete Design Matrix Objs
...
class PredictMode (ProgramMode):
    ...
    def PredictModel(self,Networks):
        ...
        for I in range (0,self.n_files,self.groupSize):# In a given group
            ...
            # Run predictions
            modelPrediction = Networks.MODEL.predict(x=matrixMLP.__GetX__(),
