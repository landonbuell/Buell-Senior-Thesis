...
class ProgramMode:
    ...
    def CollectFeatures (self,fileobj):
        ...
        del(timeFeatures)
        del(freqFeatures)
        return featureMatrix   # return the feature arrays
    ...
    def ConstructDesignMatrices (self,FILES,shapes):
        ...
        for i,FILEOBJ in enumerate(FILES):
            ...
            x1 = self.CollectFeatures(FILEOBJ)         # collect features
            X1.AddSample(x1,i)          # add to CNN matrix           

        return X1      # return 2 Design matricies & target matrix
...
class TrainMode (ProgramMode):
    ...
    def TrainModel(self,Networks):
        ...
        for I in range (0,self.n_files,self.groupSize):    # In a given group
            ...
            matrixSXX = self.ConstructDesignMatrices(FILES,shapes)
            
            # Execute training
            modelHistory = Networks.MODEL.fit(  
                    x=matrixSXX.__GetX__(),
                    y=matrixSXX.__GetY__(),
                    batch_size=32,epochs=self.n_epochs,verbose=1) 

            # Post-Training
            del(matrixSXX)            # delete Design Matrix Objs
...
class PredictMode (ProgramMode):
    ...
    def PredictModel(self,Networks):
        ...
        for I in range (0,self.n_files,self.groupSize):# In a given group
            ...
            # Run predictions
            modelPrediction = Networks.MODEL.predict(x=matrixSXX.__GetX__(),
