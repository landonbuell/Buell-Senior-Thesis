...
The architecture of the HNN enables us to perform multimodal learning and classification of musical on digital audio samples. The multimodal nature comes from two distinct input layers that each accept information from a different representation of the same sample. These different modes of data are passed through a appropriate designed neural-network that learns a set of parameters to map that mode to an output. This combination of model and modes enables this neural network to learn a more robust set of parameters for clasification of unseen labels \cite{Li}. The combined architecture is implemented using Google's \textit{Tensorflow} library for Pytho and allows the model to be trained conveniently as a single neural network \cite{Tensorflow}. All parameters and layers in the network use the same optimizer, number of epochs, and objetive function. This HNN contains 319,365 trainable parameters stored a single precision floating-point numbers.
...
In the \textit{end-to-end machine work flow} it is standard practice to divide a full data set into a \textit{training} subset and a \textit{testing} subset \cite{Geron,James,Virtanen}. The training subset is used to train the model by optimizing the parameters, $\Theta$, as to minimize the cost function, $J$, for that subset. The testing subset is used to evaluate and quantify how well optimization process performed by generating predictions on data that it has never interacted with \cite{Geron}. The training subset does contain corresponding labels but they are withheld from the trained model to make unbiased predictions, while used by the evaluation metrics to gauge performance.

We expand this idea to a common resampling method called \textit{K-Folds Cross Validation}, or X-validation for short \cite{James}. X-validation divides a data set into $K$ equally-sized subsets, called folds. The first $K-1$ folds are used to train the model, and the last fold is used as the training set. This process repeats of all $K$ folds, using a freshly 'untrained' model each time, often with slightly different initial parameters as well.

The result of X-validation is $K$ neural network models that have all been trained and evaluated on overlapping subsets of the full data set. Each model contains a different collection of learned parameters, $\Theta$ that represent a possible outcome if trained on the given subset \cite{Geron,James}. By repeating this and comparing the performance across each fold, we understand how the chosen architecture and hyper-parameters influence the stability of the performance of the network. 

Across each fold, we expect to observe a reasonably high, and consistent performance. A model that does not perform well on all of the splits indicates a poor choice or architecture or features, and a model that performs inconsistently across the splits indicates a model that suffers from high-variance, where small changes in initial conditions can drastically affect it's performance abilities \cite{James}. When consistent and strong performance across multiple splits is observed, we develop confidence that we have a model that can train effectively, and generalize appropriately \cite{Geron}. 
...
A confusion matrix is a square array that counts the number of times a sample from one class was predicted to be another class. For a $k$-categories classifier, the confusion matrix, $C$ has shape $k \times k$ where $C[i,j]$ is the number of times a sample known to be in class $i$ was predicted to be in class $j$ \cite{Geron}. A value added to the main-diagonal indicates a \textit{correct} classification, while a value on the off-diagonal indicates an \textit{incorrect} classification. We expect a well-performing classifier model to produce a diagonally-dominant confusion matrix for it's testing set.
...
We have chosen to run a $K = 10$ fold cross validation program on our model, and compute the value of the each of the four metrics described in section (\ref{subsec:Metrics}). We have also run X-validation on two similar models that represent either mode of the network in isolation. The first variant uses solely a 2D spectrogram input fed through an identical CNN + MLP as shown on the left of Fig. (\ref{fig:Architecture}). The activations are passed directly to the output layer where a prediction is made using this mode alone. Similarly, the second variant uses just a 1D feature-vector input fed through an identical MLP as shown on the right side of Fig.(\ref{fig:Architecture}). The activations also bypass the concatenation layer, and are passed directly to the output layer where a prediction is made using only this mode. This allows us to produce an approximate comparison of how the HNN compares to the CNN and MLP models individually. Cross validation was performed on each model using a different set of random splits from the same data set. The reported scores are averaged over all samples in each training data subset.
...
\textcolor{red}{Should I Keep the details about the Chaotic Synths? It seem's out of place in the scheme of this paper.}
...
Multimodal learning itself shows great promise to machine learning algorithms in numerous domains. For data sets that lend themselves to multiple representations of a single sample, the ability to leverage those different modes allows for a more comprehensive representation of data. Across full data set, this will enable a model to generate a more stable and substantial set of parameters for classification which has the potential to yield better performance. Multimodal learning does not limit itself to classification of digital audio, but instead opens the door to many genres of machine learning such a medicine, finance, and other media classification.
 
The results of cross validation indicate that we have constructed a hybrid neural network that accept two input modes, and can effectively classify the contents of a digital audio file as one of $37$ different musical instruments. The design of this HNN enables the input of two non-compatible modal representations of a sample to be processed and combined to form a single output prediction. We find that this model demonstrates better and more consistent performance at musical instrumental classification when compared to that of either the constituent CNN or MLP models. By transforming each input sample into multiple representations of itself, the HNN learns a more robust set of parameters that can more thoroughly differentiate each class. While some common misclassfications are present, many of them are due to the related nature of musical instruments, which may also cause similar confusion in human listeners. 

\textcolor{red}{I sure cannot write a conclusion to save my life. Plz Halp.}
